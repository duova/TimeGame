// Copyright (C) Varian Daemon 2023. All Rights Reserved.


#include "IO_GameplayAbilityReferences.h"

#include "AC_GASHelper.h"

TArray<TSoftClassPtr<UGameplayAbility>> UIO_GameplayAbilityReferences::GetAbilitiesForEvent(FGameplayTag Event)
{
	TArray<TSoftClassPtr<UGameplayAbility>> FoundAbilities;

	if(!Event.IsValid())
	{
		return FoundAbilities;
	}

	for(auto& CurrentAttribute : Abilities)
	{
		if(CurrentAttribute.Value.HasTagEvent(FGameplayTagContainer(Event)))
		{
			FoundAbilities.Add(CurrentAttribute.Key);
		}
	}

	return FoundAbilities;
}

bool UIO_GameplayAbilityReferences::HasAnyTagEvents(FGameplayTagContainer Events)
{
	if(Events.IsEmpty())
	{
		return false;
	}
	
	for(auto& CurrentAbility : Abilities)
	{
		if(CurrentAbility.Value.HasTagEvent(Events))
		{
			return true;
		}
	}

	return false;
}

TArray<FString> UIO_GameplayAbilityReferences::VerifyData_Implementation(UDA_CoreItem* ItemAsset)
{
	TArray<FString> Errors;
	
	for(auto& CurrentAbility : Abilities)
	{
		for(auto& CurrentGrantEvent : CurrentAbility.Value.GrantEvents)
		{
			if(CurrentAbility.Value.RemoveEvents.HasTagExact(CurrentGrantEvent))
			{
				Errors.Add(FString::Printf(TEXT("GrantEvent and RemoveEvent contain the same event %s for ability %s"), *CurrentGrantEvent.ToString(), *CurrentAbility.Key.ToString()));
			}
		}
	}
	
	return Errors;
}
