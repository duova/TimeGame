...
#include "IO_GameplayEffectReferences.generated.h"
...
UCLASS(DisplayName = "GAS Effects")
class IFP_GAS_API UIO_GameplayEffectReferences : public UItemTrait
{
	GENERATED_BODY()
...
public:
	...
	TMap<TSoftClassPtr<UGameplayEffect>, FGASTagEvents> Effects;

	/**Get all effects that are listening for the @Event*/
	UFUNCTION(Category = "Attribute Set References", BlueprintCallable, BlueprintPure)
	TArray<TSoftClassPtr<UGameplayEffect>> GetEffectsForEvent(FGameplayTag Event);

	/**Go through all effects and check if any contain
