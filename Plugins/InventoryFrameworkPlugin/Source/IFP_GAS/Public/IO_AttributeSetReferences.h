...
#include "IO_AttributeSetReferences.generated.h"
...
UCLASS(DisplayName = "GAS Attribute sets")
class IFP_GAS_API UIO_AttributeSetReferences : public UItemTrait
{
	GENERATED_BODY()
...
public:
	...
	TMap<TSoftClassPtr<UAttributeSet>, FGASTagEvents> AttributeSets;

	/**Get all attribute sets that are listening for the @Event*/
	UFUNCTION(Category = "Attribute Set References", BlueprintCallable, BlueprintPure)
	TArray<TSoftClassPtr<UAttributeSet>> GetAttributeSetsForEvent(FGameplayTag Event);
	
	/**Go through all attributes and check if any contain
